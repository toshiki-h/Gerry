	...
	def get_changes(self, day):
		...
		while more_changes:
			...
			if response.status_code >= 200 and response.status_code < 300:
				...
				log.error('GET changes between %s and %s (offset %i) failed with HTTP status %i' % (from_datetime, to_datetime, offset, response.status_code) )
			...
			for index, change_number in enumerate(change_numbers):
				...
				if response.status_code >= 200 and response.status_code < 300:
					try:						
						...
						log.error('Reading JSON for change %i failed' % (change_number) )
				else:
					log.error('GET change %i failed with HTTP status %i' % (change_number, response.status_code) )
...
if __name__ == '__main__':
	...
	parser.add_argument('--directory', dest='directory', default='./gerry_data/')
	...
	gerry_crawler = GerryCrawler(args.gerry_instance, data[args.gerry_instance]['url'], data[args.gerry_instance]['start_datetime'], datetime.datetime(2018, 7, 1), args.directory)
